...
class TestNormalizeKplResponse(unittest.TestCase):
    ...
    def test_kpl41_current_layout_uses_plate_name_and_pct_columns(self):
        from news_harness.kaipanla_targets import normalize_kpl_response
        themes = normalize_kpl_response("KPL-41", {
            "errcode": "0",
            "list": [["801001", "芯片", 5283, 0.69]],
        })
        self.assertEqual(themes[0]["theme_id"], "801001")
        self.assertEqual(themes[0]["theme_name"], "芯片")
        self.assertEqual(themes[0]["pct_change"], 0.69)
    ...
    def test_ladder_response_unwraps_stock_rows(self):
        from news_harness.kaipanla_targets import normalize_kpl_response
        stocks = normalize_kpl_response("KPL-49", {
            "errcode": "0",
            "info": [[[
                "002209", "达意隆", 0, "", 0, "中报增长",
                0, 0, 0, 0, 0, 0, "机器人概念", 0, 0, 1, 1, 0, "", "801572",
            ]]],
        })
        self.assertEqual(stocks[0]["symbol"], "SZ002209")
        self.assertEqual(stocks[0]["upstream_theme_id"], "801572")

    def test_schema_drift_reports_missing_field_path(self):
        from news_harness.kaipanla_targets import validate_kpl_response
        with self.assertRaisesRegex(ValueError, r"\$\.[A-Za-z]+"):
            validate_kpl_response("KPL-41", {"errcode": "0"})
...
class TestNormalizeSymbol(unittest.TestCase):
    ...
    def test_malformed_long_code_is_rejected(self):
        from news_harness.kaipanla_targets import normalize_symbol
        self.assertEqual(normalize_symbol("002209600785"), "")
...
class TestKplIdentityFiles(unittest.TestCase):
    def test_device_id_file_is_read(self):
        from news_harness.kaipanla_targets import _identity_override
        with tempfile.TemporaryDirectory() as tmp:
            secret_path = Path(tmp) / "device-id"
            secret_path.write_text("device-from-file\n", encoding="utf-8")
            with patch.dict(os.environ, {"TEST_KPL_FILE": str(secret_path)}, clear=False):
                self.assertEqual(_identity_override("TEST_KPL_VALUE", "TEST_KPL_FILE"), "device-from-file")


class TestHourlyTargetSetValidation(unittest.TestCase):
    def test_duplicate_symbols_fail_runtime_contract(self):
        from news_harness.kaipanla_targets import validate_hourly_target_set
        target_set = {
            "object_type": "HourlyTargetSet", "status": "ok", "stock_count": 2,
            "target_stocks": [{"symbol": "SH600000"}, {"symbol": "SH600000"}],
            "endpoint_results": [], "output_hash": "abc",
        }
        with self.assertRaisesRegex(ValueError, "duplicate target symbol"):
            validate_hourly_target_set(target_set)
